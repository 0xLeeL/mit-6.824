...
public class RollbackDataHandler implements Handler {
    private final DBServiceUseCase dbServiceUseCase;

    public RollbackDataHandler(DBServiceUseCase dbServiceUseCase) {
    ...
    public Object handle(String requestJson) {
        ...
        if (putRequest != null) {
            return dbServiceUseCase.rollback(putRequest.key(),putRequest.value());
        }
        return false;
    }
}
